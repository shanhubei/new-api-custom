...
import { deleteNewuser } from '../api'
import { ERROR_MESSAGES, SUCCESS_MESSAGES } from '../constants'
import { useNewusers } from './newusers-provider'

export function NewusersDeleteDialog() {
  const { t } = useTranslation()
  const { open, setOpen, currentRow, triggerRefresh } = useNewusers()
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true)
    try {
      const result = await deleteNewuser(currentRow.id)
      if (result.success) {
